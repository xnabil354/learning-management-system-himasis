...
export async function generateMetadata({
  ...
  if (!cert || cert.status !== "valid") {
    return {
      title: "Certificate Not Found — SMARTSIS",
      description: "This certificate could not be verified.",
    };
  }

  return {
    title: `Certificate Verified — ${cert.studentName} | SMARTSIS`,
    description: `This certificate confirms that ${cert.studentName} has successfully completed "${cert.courseTitle}" on SMARTSIS (HIMASIS E-Learning).`,
    openGraph: {
      title: `✅ Verified Certificate — ${cert.studentName}`,
      description: `Successfully completed "${cert.courseTitle}" on SMARTSIS Academy.`,
      type: "website",
    },
  };
}
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-6">
      <div className="max-w-lg w-full">
        ...
        <div className="bg-[#0F0F10] border border-slate-200 rounded-2xl overflow-hidden shadow-2xl">
        ...
        <div className="mt-6 text-center space-y-3">
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-slate-500 text-sm font-medium">
              SMARTSIS Academy — HIMASIS E-Learning
            </span>
          </div>
          ...
          {cert.courseSlug && (
            <Link
              ...
              className="inline-flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-300 transition-colors"
            >
              View Course
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-6">
      <div className="max-w-md w-full text-center space-y-6">
        ...
        <p className="text-xs text-slate-700 font-mono bg-white border border-slate-200 rounded-lg px-4 py-2 inline-block">
          ID: {id}
        </p>
        ...
        <div className="pt-2">
          <Link
            href="/"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium bg-blue-700 hover:bg-blue-600 text-white transition-colors"
          >
            Go to SMARTSIS Homepage
          </Link>
        </div>
      </div>
    </div>
